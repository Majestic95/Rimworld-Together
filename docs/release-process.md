...
1.6/
  Assemblies/
    RTClient.dll              <- upstream prebuilt, copied from Source/Assemblies/
    RTNetwork.dll             <- upstream prebuilt, copied from Source/Assemblies/
    RTShared.dll              <- upstream prebuilt, copied from Source/Assemblies/
    MessagePack.dll, Newtonsoft.Json.dll, DiscordRPC.dll, etc.
                              <- pulled from NuGet by Source/ClientDeps/, net472
                                 target, versions match upstream's csproj
...
3. Wipes `1.6/Assemblies/` for a clean dep build.
4. Runs `dotnet build Source\ClientDeps\ClientDeps.csproj -c Release`.
   This empty project pulls net472-target NuGet dependency DLLs
   (MessagePack, Newtonsoft.Json, DiscordRichPresence, plus transitive)
   into `1.6/Assemblies/`.
5. Copies upstream's prebuilt `RTClient.dll`, `RTNetwork.dll`, `RTShared.dll`
   from `Source/Assemblies/` over the top.
6. Stages a clean mod-folder snapshot into `dist\snapshot\`.
7. Strips `ClientDeps.dll` (the empty dep-puller assembly) and any
   leftover legacy `GameClient.dll` from staging.
8. Strips `.pdb` / `.xml` cruft.
9. Generates `dist\manifest.json` with SHA-256 of every file.
10. Zips the snapshot to `dist\rt-mp-<tag>.zip`.
11. Prints the exact `gh release create` command to run next.
...
| `-SkipBuild` | Skip `dotnet build` of ClientDeps. Use when iterating on packaging logic. The existing `1.6/Assemblies/` contents are reused. |
...
- **The `Source/` tree.** Source stays in git. The release ships only the
  mod-folder layout.
- **Dev artifacts** (`Dockerfile`, `Scripts/`, `docs/`, local-only tooling
  directories, etc.). Not part of the mod folder, not in the zip.
