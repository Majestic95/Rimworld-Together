...
try {
    ...
    if ($tagsMatch -and $hashesOk) {
        ...
        $staging = Join-Path $CacheDir ($remoteTag + '-staging')
        Expand-ToStaging -ZipPath $assets.Zip -StagingDir $staging

        Write-Log 'Verifying extracted files against manifest...'
        if (-not (Test-FolderMatchesManifest -Root $staging -Manifest $manifest)) {
            Fail 'Extracted snapshot did not match manifest. Aborting before touching the live install.'
        }

        Invoke-AtomicSwap -LiveDir $modFolder -StagingDir $staging
        Set-Content -Path (Join-Path $modFolder '.installed-tag') -Value $remoteTag -Encoding utf8
